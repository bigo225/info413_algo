...
def final(data, centres, distance):

    distances = calculerDistance(centres,data, distance)
    classe = affectClass(distances)
    nouvCentres = nouveauCentres(data,centres,classe)

    while nouvCentres != centres:
        centres=copy.deepcopy(nouvCentres)
        distances = calculerDistance(centres,data, distance)
        classe = affectClass(distances)
        nouvCentres = nouveauCentres(data,centres,classe)
    
    print "dfghjklkkkkkkkkkkkkkkkkkkkkkkkk"
   # if nouvCentres == centres:
  #      return nouvCentres, classe
 #   else:
#        (nouvCentres,classe)=final(data, centres, distance)
...
#---------menu------------
print "++++++++++++++++++++++ BONJOUR, Bienvennu à K-MEANS ++++++++++++++++++++++"
print ""
...
choix2=input("Veuillez le numero de la distance que vous souhaitez: ")
print ""
k=input("saisissez la valeur de k: ")
print "" 
print "1. données IRIS"
print "2. données générées"

choix1=input("Veuillez choisir un numero: ")
...
if choix1==1:
    ...
    l=input("entrez le nombre de ligne: ")
    n=input("entrez le nombre de colonne: ")
    data=generate_random_data(l,n)
    if choix2==1:
        distance="euclidienne"
    elif choix2==2:
        distance="manhattan"
        ...
        distance="minkowski"
dataSorti=copy.deepcopy(data)    
centres=initialise(data, k)
(finalCentres,finalClasse) = final(data, centres, distance)

write_data(dataSorti, "données d'entrée.csv")
write_data(finalCentres, "centres.csv")

for i in range(len(data)):
    dataSorti[i].append(finalClasse[i])

write_data(dataSorti, "données affectées.csv")
